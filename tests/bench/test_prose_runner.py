...
def test_recall_is_reported_honestly_without_claiming_to_beat_embeddings() -> None:
    ...
    assert "T2" in note and "Stack G" in note  # forward-reference, not parity claim
...
def test_checked_in_prose_artifact_shows_floor_win_and_honest_recall() -> None:
    ...
    # The prose run is byte-exact (T0 + full-dump, no embedder), so the checked-in
    # numbers must match a clean run exactly, modulo the latency section.
    assert _drop_latency(artifact) == _drop_latency(fresh)
